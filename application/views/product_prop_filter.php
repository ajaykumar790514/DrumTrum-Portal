...
<div class="form-group">
    ...
    <div class="size-box <?= $i == 1 ? 'prop-box'.$row[0]->id : 'prop-box'.$row[0]->id; ?>">
        <ul>
            ...
            ?>">
                <button type="button" class="prop_btn" onclick='select_props(this, <?= $prod_id; ?>, "<?= $row2->select_id; ?>", <?= $props_all_id; ?>)'><?= $row2->select_value; ?></button></li>
            <?php
            $flag=1;
            endif;
            endforeach;
    ...
    <div class="form-group">
        ...
        <div class="size-box <?= $i == 1 ? 'prop-box'.$row[0]->id : 'prop-box'.$row[0]->id; ?>">
            <?php
                ...
                $props_all_id = json_encode($props_id_all);
            ?>
            <select class="form-control form-select" onchange='select_props_dropdown(this, <?= $prod_id; ?>, <?= $props_all_id; ?>)'>
                <?php 
                // print_r($props_master_id);            
                $flag=0;
                foreach($product_selectable_props as $rowArr):
