...
//                    $(`#cart_btn${pid}`).attr('onclick', `add_to_cart(${pid},this)`).html(`<i class="fi-rs-shopping-bag-add"></i>`).fadeIn('slow');
			    ...
			    if( cart_id ){
				    $.ajax({
					    ...
					    success: function(data){
						   ...
						   let rounded_total = Math.round(total_payable);
					        		// $('button.make-online-payment').text(`Pay Now <?//= $shop_detail->currency; ?>${rounded_total}`);
									$("#subamount").val(rounded_total);
					        	});
					        }
    ...
    // Check if item_qty is a positive value
    if (item_qty >= 0) {
        $.ajax({
        ...
        // Handle the case where item_qty is negative (show an error, log, or any other appropriate action)
        toastr.error('Error: item_qty should be a positive value.');
    }
}
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.trim() == 'SUCCESS') {
                    $("#available-msg").html("<h3 class='text-success mt-1'>Delivery available here in this pincode.</h3>");
                } else {
                    $("#available-msg").html("<h3 class='text-danger mt-1'>Delivery not available in this pincode. Kindly check your pincode.</h3>");
                }
            }
        });
    });
