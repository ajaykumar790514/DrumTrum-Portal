<div class="row">
	...
	?>
     ...
     <div class="col-6 p-0 h-100">
                        <div class="product-box">
                            ...
                            <div class="product-info product-content">
                                ...
                                <?php if ($result['mrp'] > $selling_rate) {?>
                                    <h4 class="mt-4 newheight"><del class="d-save text-danger"><?= $shop_detail->currency; ?><span class="d-mrp text-danger"><?php echo number_format((float)($result['mrp']), 2, '.', '') ;?></span></del></h4>
                                    <?php }?>
                               
                               <?php if(!empty($offers))
                          {?>
                      <h4 class="text-dark "><?= $shop_detail->currency; ?><span class="d-rate"> <?php echo bcdiv($deatailfinalper, 1, 2);?></span></h4>
                         <?php  }else{?>
                            <h4 class="text-dark"><?= $shop_detail->currency; ?><span class="d-rate"><?php echo @bcdiv($selling_rate, 1, 2);?></span></h4>
                        <?php }?>
        ...
        <div class="col-lg-4 p-0 padding-cls">
            <div class="product-box full-box">
                <div class="img-block">
                    ...
                    <div class="lable-wrapper">
                        <?php
                                $offerss1 = $this->product_model->get_data('shops_coupons_offers','product_id',$result['product_id']);
                                foreach($offerss1 as $offer)
                                {
                                if($offer->discount_type==1)
                                {
                                    $deatailoffervalue=   $offer->offer_associated.' % OFF';
                                    $deatailoffertype=$offer->discount_type;
                                     $deatailfinalper1 = $result['mrp']*$offer->offer_associated/100;
                                     $deatailfinalamount = $result['mrp']-$deatailfinalper1;
                                }else
                                {
                                    $deatailoffervalue ='Only '.$shop_detail->currency.'  '.$result['mrp']-$offer->offer_associated;
                                    $deatailoffertype=$offer->discount_type;
                                    $deatailfinalper1 = ($result['mrp']-$offer->offer_associated);
                                     $deatailfinalamount = $result['mrp']-$deatailfinalper1;
                                }    
                ...
                <div class="product-info product-content">
                    ...
                    <a href="<?=base_url('product-detail/'.$result["inventory_id"].'/'._encode($result["category_id"]).'/'._encode($result["category_id"]))?>"></h6><h6 class="p-name mt-1"><?=$result['name'] ?></h6></a>
                    <?php if ($result['mrp'] > $selling_rate) {?>
                                    <h4 class="mt-4 newheight"><del class="d-save text-danger"><?= $shop_detail->currency; ?><span class="d-mrp text-danger"><?php echo number_format((float)($result['mrp']), 2, '.', '') ;?></span></del></h4>
                                    <?php }?>
                               ...
                               <?php if(!empty($offers))
                          {?>
                      <h4  class="text-dark"><?= $shop_detail->currency; ?><span class="d-rate"><?php echo bcdiv($deatailfinalper1, 1, 2);?></span></h4>
                         <?php  }else{?>
                            <h4  class="text-dark"><?= $shop_detail->currency; ?><span class="d-rate"><?php echo bcdiv($selling_rate, 1, 2);?></span></h4>
                        <?php }?>
        ...
        <div class="col-lg-4 col-12">
            <div class="container ps-0 padding-cls">
                <div class="row row-border2">
    <?php endif; if ($i > 5): ?>
                    ...
                    <div class="col-6 p-0">
                        <div class="product-box">
                            <div class="img-block">
                                ...
                                <div class="lable-wrapper">
                                    <?php
                                      $offerss2 = $this->product_model->get_data('shops_coupons_offers','product_id',$result['product_id']);
                                foreach($offerss2 as $offer)
                                {
                                if($offer->discount_type==1)
                                {
                                    $deatailoffervalue=   $offer->offer_associated.' % OFF';
                                    $deatailoffertype=$offer->discount_type;
                                     $deatailfinalper2 = $result['mrp']*$offer->offer_associated/100;
                                     $deatailfinalamount = $result['mrp']-$deatailfinalper2;
                                }else
                                {
                                    $deatailoffervalue ='Only '.$shop_detail->currency.'  '.$result['mrp']-$offer->offer_associated;
                                    $deatailoffertype=$offer->discount_type;
                                    $deatailfinalper2 = ($result['mrp']-$offer->offer_associated);
                                     $deatailfinalamount = $result['mrp']-$deatailfinalper2;
                                }    
                            ...
                            <div class="product-info product-content">
                               ...
                               <?php if(!empty($offers))
                          {?>
                      <h4  class="text-dark"><?= $shop_detail->currency; ?><span class="d-rate"><?php echo bcdiv($deatailfinalper2, 1, 2);?></span></h4>
                         <?php  }else{?>
                            <h4  class="text-dark"><?= $shop_detail->currency; ?><span class="d-rate"><?php echo bcdiv($selling_rate, 1, 2);?></span></h4>
                        <?php }?>
                                <div class="rating three-star mt-1">
                                ...
                                <div class="addtocart_detail">
                                    <div>
                                        ...
                                        <div class="product-action-1 show add-to-cart-div-<?=$result['inventory_id']?> addtocart_btn" style="position:static">
                                                ...
                                                <a aria-label="Add To Cart" class="action-btn hover-up" id="cart_btn<?=$result['inventory_id']?>" onclick="<?= $cart_onclick;?>" href="javascript:void(0)"><i class="fa fa-shopping-bag"></i> add to cart</a>
                                                <?php
                                                        endif;
                                                        else:
                                                ?>
