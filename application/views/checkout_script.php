...
<script type="text/javascript">
    ...
    //             // console.log(data);
    //             if (data == 'SUCCESS') {
    //                 $("#available-msg").text("Service available here in this pincode.");
    //             }else{
	...
	$(document).on('click', '.make-online-payment',function(){
		$('.loader-container').show();
		 ...
		 $.ajax({
            ...
            data:{aid:id},
// 			beforeSend: function() {
//     button.attr('disabled', true).text('Please Wait').css({
//         'padding': '6px 8px',
//         'color': 'black'
//     });
// },
                ...
                if (data == 'SUCCESS') {
					...
					success:function(data){
						...
						}else
						{
							...
							$(".checkout_cart").load('<?=base_url()?>checkout/checkout_cart');
	...
	//     if(t_value >=20){ 
	    //  makeOnlineDirectPayment(id,this);
	//    }else
	//    {
	//    	//$("#minimum-value").css('display','block');
	//    	alert("minimum spend £20");
	//    }
	    
	    //below function will use when need to check delivery criteria beforw making payment
		//const res=check_addressforworldpay(id,this);
	});
                 ...
                 $.ajax({
                                ...
                                success: function(res){
                    	            ...
                    	            var options = {
                    	                ...
                    	                "currency": "INR",
                    	                "name": "DrumTrum",
                    	                "description": "DrumTrum online payments",
	...
	function make_cod_payment(cod_limit,total_value) 
	{
	    ...
	    //     $(".pay-btn-cod").html('PAY NOW ' +total_value+' <i class="icofont-long-arrow-right"></i>');
	    //     return false;
	    // }else{
	    	let aid = $('.address-div').find('button.delivery-btn.btn-success').val();
	    	if(!aid){
...
//                //	            	return false;
    ...
    function check_address(id){
		...
		let d_charge = 0.00;
		$.ajax({
			url:"<?=base_url('home/check_address_default');?>",
			method:"POST",
			data:{
				id:id
			},
		...
		$.ajax({
			...
			success:function(response){
				...
				if (response.charge) {
					d_charge = parseFloat(response.charge); // Ensure 'charge' is parsed as a float
                    subTotal = total + d_charge;
				}

				if (total > eli_amnt) {
					$(".delivery-charge").text(response.charge);
					$(".sub-total").text(response.amount);
					$(".to-pay").text(response.finalamount);
				$(".eligible-text").hide();
			} else {
