...
class Product_model extends CI_Model
{
    ...
    {
        $this->db
        ->select('t1.*,t1.name as prod_name,t4.img,t4.id as cover_id,t3.mrp,tf3.selling_rate,t5.name as cat_name,t3.qty as product_qty,t6.offer_upto,t6.discount_type,t7.is_featured')
   ...
   public function get_value($id)
    ...
    public function get_product_by_id($pid)
    {
        ...
        $this->db
        ->select('t1.*,t1.name as prod_name,t2.img,t2.thumbnail,t2.is_cover,t2.id as cover_id, IFNULL(t3.mrp, 0) as mrp, t3.id as inventory_id, t3.selling_rate, t3.qty, t4.product_id as wishlist_pid, t5.id as cart_id, IFNULL(t5.qty, 0) as cart_qty, t6.discount_type, t6.offer_upto, t7.name as flavour_name,t6.offer_associated')
        ->from('products_subcategory t1')
        ->join('products_photo t2', 't2.item_id = t1.id','left')        
        ->join('shops_inventory t3', 't3.product_id = t1.id','left')        
        ->join('wishlist t4', 't4.product_id = t1.id','left')      
        ->join('cart t5', "t3.id=t5.product_id AND t5.user_id='".$user_id."'",'left')
        ->join('shops_coupons_offers t6', 't6.product_id = t1.id','left')
        ->join('flavour_master t7', 't7.id = t1.flavour_id AND t7.is_deleted="NOT_DELETED" AND t7.active="1"','left')
        ->where(['t2.is_cover' => '1','t3.id' => $pid,'t1.is_deleted'=>'NOT_DELETED','t1.active'=>'1'])
	...
	{
		$query = $this->db
        ->select('t1.selling_rate, t1.mrp, t2.tax_value as product_tax, t1.qty as stock_qty,t2.id as pid, t2.name as product_name, t2.rating, t3.img, t4.offer_upto, t4.discount_type, t5.id as cart_id, t2.unit_type, t2.unit_value, t6.name as flavour_name,t2.product_code')
        ...
        ->join('flavour_master t6', 't6.id = t2.flavour_id AND t6.is_deleted="NOT_DELETED" AND t6.active="1"','left')
        ->where(['t1.is_deleted' => 'NOT_DELETED','t2.is_deleted'=>'NOT_DELETED','t2.active'=>'1','t3.is_cover' =>'1','t1.id' =>$pid])
		->get();
		return $query->row();
	}
...
//t2.is_parent
    ...
    {
        $this->db
        ->select('t1.*,t1.id as pid,t1.parent_cat_id,t3.id as inventory_id, t4.thumbnail,t5.name as flavour')
        ->from('products_subcategory t1')
    ...
    {
        $this->db->select('*')
        ->from('products_mapping as t1')
        ->join('products_subcategory as t2','t2.id=t1.map_pro_id')
        ->where(['t2.is_deleted'=>'NOT_DELETED','t2.active'=>'1','t1.pro_id'=>$id]);
        return $this->db->get()->result();
    ...
    {
        $this->db->select('t1.props_id,t2.id,t2.name,t2.display_type,t3.id as value_id,t2.view_type')
        ->from('product_props t1')
        ->join('product_props_master t2','t1.props_id=t2.id')
        ->join('product_props_value t3', 't3.id = t1.value_id')
        ->where('t2.is_selectable','3')
        ->where('t1.product_id',$id)
        ->where('t1.is_deleted','NOT_DELETED')
    ...
    //written by techfi for fetching next level props value like size after color
    public function getPropsValueNextLevel($mapProducts=null,$value_id=null,$propIdFirst=null,$propIdSecond=null)
    {
        $query=$this->db->select('t1.*')
        ->from('product_props t1')
        ->where(['t1.props_id'=>$propIdFirst,'t1.value_id'=>$value_id])
        ->group_start()
        ->where_in('t1.product_id',$mapProducts)
        ...
        //now get all next level props based on prevoius one    
        $this->db->select('*, t2.value as select_value, t2.id as select_id , t2.seq as vseq,t4.qty')
        ->from('product_props t1')
        ->join('product_props_master t3','t1.props_id=t3.id')
        ->join('product_props_value t2', 't2.id = t1.value_id')
        ->join('shops_inventory t4', 't4.product_id = t1.product_id')
        ->where(['t1.props_id'=>$propIdSecond,'t4.status'=>1, 't4.is_deleted'=>'NOT_DELETED'])
        ->group_start()
        ->where_in('t1.product_id',$product_ids)
         ->group_end()
        ->order_by('t2.seq','asc');
        $query=$this->db->get();
    ...
    {
        $this->db->select('*, t2.value as select_value, t2.id as select_id , t2.seq as vseq,t2.other_value as colorcode,t4.qty')
        ->from('product_props t1')
        ->join('product_props_master t3','t1.props_id=t3.id')
        ->join('product_props_value t2', 't2.id = t1.value_id')
        ->join('shops_inventory t4', 't4.product_id = t1.product_id')
        ->where(['t4.status'=>1, 't4.is_deleted'=>'NOT_DELETED'])
        //->where('t1.is_deleted','NOT_DELETED')
        //->where('t3.is_selectable','3')
        ->group_start()
        ->where_in('t1.product_id',$product_id)
        ->group_end()
        ->order_by('t2.seq','asc');
        $query=$this->db->get();
        $result=($query->num_rows() > 0) ? $query->result() : false;
   ...
   function fetch_data($category_pro_id,$pro_id,$id,$cids,$shop_id,$brand,$prop_filter,$sort_by,$limit,$start,$is_count,$isAll)
    {
        ...
        ->select('t1.*, t1.name as prod_name, t4.img,t4.thumbnail, t4.id as cover_id, t3.id as inventory_id, t3.mrp, t3.selling_rate, t3.qty as product_qty, t6.offer_upto, t6.discount_type, t10.name as flavour_name')
        ->from('products_subcategory t1')  
        ->join('shops_inventory t3','t1.id=t3.product_id AND t3.is_deleted="NOT_DELETED" AND t3.status="1"','left')
        ->join('products_photo t4', 't4.item_id=t1.id AND t4.active="1" AND t4.is_cover="1"','left')
        //->join('products_category t5', 't1.parent_cat_id = t5.id')
...
//        }
        ...
        {   
            $this->db->where_in('t1.id' , $pro_id);
            $this->db->group_start();
            $this->db->where_in('t11.value_id', $prop_filter);
            $this->db->group_end();
        ...
        if(!empty($prop_filter))
            $this->db->group_by('t11.product_id');
    ...
    function get_similer_products($pro_id,$shop_id)
    {
        $this->db
        // ->select('t1.id,t1.parent_cat_id')
        ->select('t1.*, t1.id as product_id, t1.name as prod_name, t4.img,t4.id as cover_id, t4.thumbnail, t3.mrp,t3.selling_rate, t3.id as inventory_id, t3.qty as product_qty, t6.offer_upto, t6.discount_type, t7.is_featured, t8.product_id as wislist_pid, t9.name as flavour_name')
        ->from('products_subcategory t1')  
        ->join('shops_inventory t3', 't1.id=t3.product_id AND t3.is_deleted="NOT_DELETED" AND t3.status="1"','left')
        ->join('products_photo t4', 't4.item_id=t1.id AND t4.active="1" AND t4.is_cover="1"','left')
        //->join('products_category t5', 't1.parent_cat_id = t5.id')
		...
		->join('product_flags t7', 't1.id=t7.product_id','left')
        ->join('wishlist t8', 't8.product_id=t1.id','left')
        ->join('flavour_master t9', 't9.id = t1.flavour_id AND t9.is_deleted="NOT_DELETED" AND t9.active="1"','left')
        ->where(['t1.is_deleted' => 'NOT_DELETED','t1.active'=>'1','t3.shop_id' =>$shop_id])
        // ->where(['t1.parent_cat_id' => $sub_cat_id]);
    ...
    public function wishlist_product_details($pid)
    {
        ...
        ->join('products_photo t2', 't2.item_id = t1.id','left')
        ->join('shops_inventory t3', 't3.product_id = t1.id','left')
        ->where(['t1.is_deleted' => 'NOT_DELETED','t2.is_cover' =>'1','t1.id' =>$pid])
        ->get();
        return $query->row();
    }
    ...
    {
        $query = $this->db
        ->select('t1.*,t2.cat_id,t3.id as inventory_id,t1.url')
        ->from('products_subcategory t1')
