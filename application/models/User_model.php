...
class User_model extends CI_Model
{
    ...
    {
        $data = $this->db->get_where('cities',['state_id' => $sid])->result();
    ...
    {
        // $query = $this->db
        // ->select('t1.id as oid,t1.*,t2.qty as item_qty,t3.name as status_name,t3.id as status_id,t4.id as product_id,t4.name as product_name,t5.img,t6.cancellation_period,t6.is_cancellation')
        // ->from('orders t1')
        ...
        // ->join('product_flags t6', 't6.product_id = t4.id','left')
        // ->where(['t1.user_id'=>$uid,'t1.shop_id'=>$shop_id,'t5.is_cover' =>'1'])
        // ->order_by('t1.added','desc')
        // ->get();
		// return = $query->result();
        return $this->db->select("t1.id as oid,t1.house_no,t1.address_line_2,t1.address_line_3,t1.pincode,t1.city,t1.country,t1.email, t1.added, t1.total_value, t1.orderid, t1.status, t1.updated,t1.added as order_date, SUM(t2.qty) as item_qty, t3.name as status_name, t3.id as status_id,t1.delivery_charges")
            ->from('orders as t1')
            ->join('order_items t2', 't2.order_id = t1.id', 'left')
            ->join('order_status_master t3', 't3.id = t1.status','left')
    ...
    $originalAmount = number_format((float)$amount, 2, '.', ''); // Keep original amount to two decimals
    $roundedAmount = number_format(round($amount, 2), 2, '.', ''); // Standard rounding to two decimals
    $decimalPart = $amount - floor($amount); // Get the decimal part of the amount

    if ($decimalPart < 0.50) {
        $newAmount = floor($amount); // Round down if less than 0.50
        $roundoff = -number_format($decimalPart, 2, '.', ''); // Negative roundoff for less than 0.50
    } else {
        $newAmount = ceil($amount); // Round up if 0.50 or more
        $roundoff = number_format($newAmount - $amount, 2, '.', ''); // Positive roundoff for 0.50 or more
    }
    ...
    {
  //       $order = $this->db
  //       ->select('t1.id as oid,t1.*,t1.added as order_date,t2.qty as item_qty,t3.name as status_name,t4.id as product_id,t4.name as product_name,t4.unit_value,t4.unit_type,t5.img,t6.fname,t6.lname,t6.mobile,t7.address,t7.contact_name,t7.contact')
  //       ->from('orders t1')
  //       ->join('order_items t2', 't2.order_id = t1.id','left')
  //       ->join('order_status_master t3', 't3.id = t1.status','left')
  //       ->join('products_subcategory t4', 't4.id = t2.product_id','left')
  ...
  //       ->where(['t4.is_deleted' => 'NOT_DELETED','t1.id'=>$oid,'t5.is_cover' =>'1'])
        ...
        $order = $this->db
        ->select('t1.id as oid,t1.*,t1.added as order_date,t3.name as status_name, t6.fname, t6.lname, t6.mobile, t7.address, t7.contact_name, t7.contact, t6.email as cust_email, db.full_name as delivery_boy_name, db.contact_number as delivery_boy_contact,t2.price_per_unit,t2.total_price,t2.qty as item_qty,t2.item_props_value,t2.product_id as ProductID,t1.delivery_charges')
        ->from('orders t1')
        ->join('order_items t2', 't2.order_id = t1.id','left')
        ->join('order_status_master t3', 't3.id = t1.status','left')
        ->join('customers t6', 't6.id = t1.user_id','left')
        ...
        $order_items = $this->db->select('t2.qty as item_qty, t2.*, t4.id as product_id, t4.parent_cat_id, t1.is_parent, t4.name as product_name, t4.unit_value, t4.unit_type, t5.img,t4.url as pro_url')
            ->from('order_items t2')
...
public function invoice_loop_details($oid)
    {
        $query = $this->db
        ->select('t1.id as oid,t1.*,t1.added as order_date,t1.tax as order_tax,t2.item_props_value,t2.qty as item_qty,t2.purchase_rate,t2.price_per_unit,t2.tax_value,t3.name as status_name,t4.sku,t4.id as product_id,t4.name as product_name,t4.unit_value,t4.unit_type,t2.discount_type,t2.offer_applied,t2.total_price,t5.name as flavour,t11.selling_rate')
        ->from('orders t1')
        ->join('order_items t2', 't2.order_id = t1.id','left')        
        ->join('order_status_master t3', 't3.id = t1.status','left')        
        ->join('products_subcategory t4', 't4.id = t2.product_id','left')  
...
//         ->from('orders t1')
//         ->join('order_items t2', 't2.order_id = t1.id','left')
//         ->join('shops_inventory si', 'si.id = t2.inventory_id')
//         ->join('order_status_master t3', 't3.id = t1.status','left')
//         ->join('products_subcategory t4', 't4.id = si.product_id','left')
// 		->join('products_photo t5', 't5.item_id = t4.id','left')
// 		->join('customers t6', 't6.id = t1.user_id','left')
...
//     }
    ...
    {
        $query = $this->db
        ->select('t1.*,t1.id as aid,t2.name as state_name,t3.name as city_name')
    ...
    {
        $query = $this->db
        ->select('t1.*,t2.offer_upto,t2.discount_type,t3.tax_value as product_tax')
    ...
    {
        $query = $this->db
        ->select('t1.id as oid,t1.*,t1.added as order_date,t1.tax as order_tax,t2.item_props_value,t2.qty as item_qty,t2.purchase_rate,t2.tax_value,t3.name as status_name,t4.sku,t4.id as product_id,t4.name as product_name,t4.unit_value,t4.unit_type,t5.img,t6.fname,t6.lname,t6.mobile as cus_mobile,t6.email as cust_email,t7.address as cust_address,t7.contact_name,t7.contact as cust_contact,t8.*,t9.name as city_name,t10.name state_name,t1.address_line_2 as address2,t1.address_line_3 as address3,t1.apartment_name,t1.floor,,t1.house_no as cust_house_no,t2.mrp,t2.price_per_unit,t1.state as cust_state,t1.email as order_email,t1.city as cust_city,t1.pincode as cust_pincode,t2.total_price,t1.remark as instructions,t6.id as customer_id,t11.selling_rate,t2.discount_type,t2.offer_applied')
        ->from('orders t1')
        ->join('order_items t2', 't2.order_id = t1.id','left')        
        ->join('order_status_master t3', 't3.id = t1.status','left')        
        ->join('products_subcategory t4', 't4.id = t2.product_id','left')        
        ->join('products_photo t5', 't5.item_id = t4.id','left')  
        ->join('customers t6', 't6.id = t1.user_id','left')  
        ->join('customers_address t7', 't6.id = t7.customer_id','left')  
        ->join('shops t8', 't8.id = t1.shop_id','left')  
        ->join('cities t9', 't9.id = t8.city','left')  
        ->join('states t10', 't10.id = t8.state','left')  
        ->join('shops_inventory t11', 't11.id = t2.inventory_id','left') 
        ->where(['t4.is_deleted' => 'NOT_DELETED','t1.id'=>$oid,'t5.is_cover' =>'1' ,'t7.is_default'=>'1'])  
        ->get();   
